...
* generate diagram of Postgres database.
...
[source,java]
----
import com.acme.starters.test.WireMockUtils;
...
=== Auto-generate Postgres database diagram

To generate Postgres database diagram, add the following property:
...
Run an integration test and view the diagram in `target/generated-docs/database.png` file.